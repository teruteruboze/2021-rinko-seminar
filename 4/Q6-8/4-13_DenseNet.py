...
if __name__ == '__main__':
    ...
    # folder creation #######################################################################
    # config file (json)
    with open('4-13_DenseNet.json') as f:
        config = json.load(f)
    # make necessary dir
    ...
    # Dataset ##############################################################################
    dataset = makeDataset.BasicDatasetTo3chEnlarge(config['training']['BATCH_SIZE'],config['path']['dataset'],config['dataset']['num_valid'], -1, 128, 128)
