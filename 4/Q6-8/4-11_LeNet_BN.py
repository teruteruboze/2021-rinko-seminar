...
if __name__ == '__main__':
    ...
    # folder creation #######################################################################
    # config file (json)
    with open('4-11_LeNet_BN.json') as f:
        config = json.load(f)
    # make necessary dir
    ...
    # Dataset ##############################################################################
    dataset = makeDataset.BasicDataset_staticSplit(config['training']['BATCH_SIZE'],config['path']['dataset'],config['dataset']['num_valid'],config['dataset']['num_train'])
