...
if __name__ == '__main__':
    ...
    # Dataset ##############################################################################
    dataset = makeDataset.BasicDatasetForGAN(config['training']['BATCH_SIZE'],config['path']['dataset'])
    ...
    # main #################################################################################
    modelGen   = makeNet.Generator_().to(device)
    modelGen.apply(makeNet.weights_init)
    modelGen.makenoise_like(device)

    modelDis   = makeNet.Discriminator_().to(device)
    ...
    for epoch in range(config['training']['EPOCH']):
        ...
        # 
        with torch.no_grad():
            fake = modelGen(modelGen.fixed_noise)
            fake = fake.view(fake.shape[0], 1, 28, 28).detach().cpu()
