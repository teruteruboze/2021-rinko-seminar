...
import util.trainDCGAN_GMDC as train
...
if __name__ == '__main__':
    ...
    # folder creation #######################################################################
    # config file (json)
    json_fname = '6-3a_ModeCollapse.json'
    ...
    # Dataset ##############################################################################
    # データセットは TrainDCGAN_GMDC.py で作ってます．
    # logger setup #########################################################################
    ...
    modelDis   = makeNet.Discriminator_GMDC(config['training']['nCH']).to(device)
    ...
    for epoch in range(config['training']['EPOCH']):
        ...
        # Train
        LossG, LossD = train.do(config, logger, modelGen, modelDis, optimizerGen, optimizerDis, loss_fn, device)
